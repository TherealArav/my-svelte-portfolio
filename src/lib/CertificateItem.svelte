...
<li>
	...
	>
		...
		<div
			class="flex w-full flex-col gap-3 border-l-2 border-slate-200 pl-4 sm:pl-8 dark:border-slate-800"
		>
			<header class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
				<h3 class="text-2xl font-bold text-slate-900 dark:text-slate-100">
					{issuer}
					<span class="block font-medium text-slate-600 sm:ml-2 sm:inline dark:text-slate-400"
						>— {title}</span
					>
				</h3>
				{#if badge}
			...
			<p class="text-xl leading-relaxed text-slate-700 dark:text-slate-300">
				...
				{#if url}
					<a
						...
						class="rounded px-1 font-medium text-purple-700 underline underline-offset-4 transition-colors hover:text-purple-800 focus:ring-4 focus:ring-purple-500 focus:outline-none dark:text-purple-400 dark:hover:text-purple-300"
					>
						View Credential
					</a>
				{/if}
